...
$parent_title = get_post(2973)->post_title;
$category = get_the_terms($page_id, 'category'); // get category
...
<main class="article-single animated-slow animated fadeInUp">
            ...
            <?php if(!empty($tags)): ?>
              <ul class="tags">
                <?php foreach ($tags as $key => $tag):?>
                  <li class="item">
                    <a href="<?php echo esc_url( home_url( 'article?post_tag=' ) ) . $tag->slug; ?>"><?php echo $tag->name; ?></a>
                  </li>
                <?php endforeach; ?>
              </ul>
            <?php endif; ?>
    ...
    ?>
        ...
        <span class="btn-warp prev <?php echo $current_key > 0 ? 'show' : 'no-show'?>">
            ...
            ?>
                <a class="previous-btn btn" href="<?php echo esc_url( home_url( 'article/' ) ) .  $prev_key->post_name; ?>">
        ...
        <span class="btn-warp goback">
            <a class="btn" href="<?php echo esc_url( home_url( 'article' ) ); ?>">返回文章目錄</a>
        </span>
        <!-- 下一篇 -->
