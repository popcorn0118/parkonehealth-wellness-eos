						...
						<tbody>
							...
							foreach ($new_checkup_set_list as $term_name => $checkup_set){
								...
								foreach ($checkup_set as $_checkup_plan_id => $checkup_set_title){
									...
									if (!$has_check_item) {
										continue; // 如果該健檢項目在所有方案裡都沒有, 就跳過不顯示
									}
...
function plan_search_result_fn()
{
	...
	error_log("plan_search_result plans_compare: " . var_export($plans_compare, true));
	error_log("plan_search_result plan_name_list: " . var_export($plan_name_list, true));
	wp_enqueue_script('jquery');
	ob_start();
?>
								...
								<tr class='compare_row'>
									...
									<td>
										<label class='p-title' for='<?= $pid; ?>'>
											...
											<?php
											// if (in_array($pname, $plans_compare)){
											// 	echo "<span class='plan-tag'>互比方案</span>";
											// }
											?>											
										</label>
...
<tr class="tfoot">
	<td colspan="3">
		<div class="info">
			...
			<!-- <button type='button' id='clear_all_compare_plans' style="display:none;">清除搜尋結果和互比方案<svg width="16" height="18" viewBox="0 0 16 18" fill="none" xmlns="http://www.w3.org/2000/svg">
...
function get_plan_search_result()
{
	...
	error_log('plan_search_result postdata: ' . var_export($postdata, true));
	// 處理性別
	$gender = "";
